      ...
      <div className="absolute inset-0 flex items-center justify-center p-4">
        <div ref={dialogRef} tabIndex={-1} className="w-full max-w-6xl max-h-[90vh] rounded-2xl bg-white shadow-2xl overflow-hidden animate-fade-in flex flex-col">
          ...
          <div className="p-6 overflow-auto">
            ...
            {((product.shortDescription && product.shortDescription.trim()) || (product.description && product.description.trim())) && (
              <div className="mt-6 space-y-2">
                {areSame ? (
                  ...
                  <>
                    {shortText && (<div className="text-body text-gray-800">{shortText}</div>)}
                    {longHtml && (<div className="prose prose-sm max-w-none text-gray-700" dangerouslySetInnerHTML={{ __html: longHtml }} />)}
                  </>
                )}
              </div>
            )}
