...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({ items: items.map((d) => ({ ...d, name: cleanCategoryName(d.name as string), _id: String(d._id), parentId: d.parentId ? String(d.parentId) : null })) });
}
...
export async function POST(req: NextRequest) {
  ...
  const res = await categories.insertOne({ name: cleaned, slug, fullPath, depth, parentId: parent?._id ?? null, createdAt: new Date(), updatedAt: new Date() });
